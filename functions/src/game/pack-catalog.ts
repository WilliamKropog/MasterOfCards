...
export type PackDefinition = {
  ...
  name: string;
  element: string;
  /** Guaranteed pulls by rarity (element-filtered). */
  guaranteed: ReadonlyArray<PackRecipeSlot>;
  /** Extra wild cards that roll rarity via RARITY_WEIGHTS within the pack element. */
  wildCount: number;
};
...
/**
 * Builds owned-card drafts for opening a catalog pack (no acquiredAt).
 */
export function generateCardsForPack(
  ...
  const drafts: Array<Omit<OwnedCardDoc, "acquiredAt">> = [];

  for (const slot of def.guaranteed) {
    for (let i = 0; i < slot.count; i++) {
      const catalogCardId = rollElementCardId(def.element, slot.rarity);
      drafts.push(generateOwnedCardFromCatalogId(catalogCardId, cardSource));
    }
  }
